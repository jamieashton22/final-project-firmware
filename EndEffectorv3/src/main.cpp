...
#define TILT_THRESHOLD 5.0 // TUNE   // don't correct tilts smaller than this 

#define SERVO_UPDATE_RATE 20.0  // TUNE  
...
unsigned long RservoPrevTime = 0.0;
unsigned long PservoPrevTime = 0.0;
float dt_imu = 0.05; 

bool roll_servo_reached = false; //flag to store if servo has reached position
bool pitch_servo_reached = false;
bool roll_servo_time_passed = false; //flag to store if servo update time reached 
...
void loop() {
  ...
  // calculate dt for ROLL servo timing 

  unsigned long RcurrTimeServo = millis();
  if(RcurrTimeServo - RservoPrevTime >= SERVO_UPDATE_RATE) { // if sufficient time passed to allow servos to reach position 
    roll_servo_time_passed = true;
  }
  
  else{
    roll_servo_time_passed = false;
  }

  // calculate dt for PITCH servo timing

  unsigned long PcurrTimeServo = millis();
  if(PcurrTimeServo - PservoPrevTime >= SERVO_UPDATE_RATE) { // if sufficient time passed to allow servos to reach position 
    pitch_servo_time_passed = true;
  }
  
  else{
    pitch_servo_time_passed = false;
  }
  ...
  if(roll_servo_reached && roll_servo_time_passed ) {

    // reset servos timer 
    RservoPrevTime = RcurrTimeServo;
  ...
  if(pitch_servo_reached && pitch_servo_time_passed ) {

    // reset servos timer 
    PservoPrevTime = PcurrTimeServo;
    ...
    if(abs(pitch_deg) > 2.0) {
      ...
      // incremental control (just P control for now)
      float pitch_correction = 0.4 * pitch_deg;
      
      pitch_write = pitch_servo_true + pitch_correction;
      pitch_write = constrain(pitch_write, 20, 170);  // safe limits
      pitchServo.write(round(pitch_write));

    }

  pitch_servo_reached = false; // reset flag 
