...
bool servo_reached = false; //flag to store if servo has reached position
...
void loop() {
  ...
  // check if servo reached position
  ...
  if(servo_reached && servo_time_passed) {
    ...
    if(abs(roll_deg) > 2.0) {
      ...
      float correction = 0.6 * roll_deg;
      
      roll_write = roll_servo_true - correction;
      roll_write = constrain(roll_write, 30, 150);  // safe limits
      rollServo.write(round(roll_write));

    }

    servo_reached = false; // reset flag 
